...
GCodeLoader.prototype = Object.assign( Object.create( Loader.prototype ), {
	...
	parse: function ( data ) {
		...
		var lines = data.replace( /;.+/g, '' ).replace( /\bn\S+/ig, '' ).replace( /\(.+/g, '' ).replace( /=R+/g, '' ).split( '\n' );
		...
		for ( let i = 0; i < lines.length; i ++ ) {
			...
			if ( cmd === 'G00' || cmd === 'G01' || cmd === 'G1' || cmd === 'G0') {
				//console.log("here")
				var line = {
				...
				if ( true ) {  //delta( state.e, line.e ) > 0
					//console.log(delta( state.e, line.e ))
					line.extruding = cmd === 'G01';  //delta( state.e, line.e ) > 0
